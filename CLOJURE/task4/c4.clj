...
(defn notify-msg
  ...
  (let [bill   (:bill state)
        ...
        ;; helper: atomically try to take n items from storage
        take-from-storage
        (fn [n]
          (if (pos? n)
            (try
    ...
    ;; try to take only what is really needed
    (let [needed   (get bill ware)
          have     (get buffer ware 0)
          missing  (max 0 (- (or needed 0) have))
      ...
      ;; try to run production cycles while possible
      (loop [buf buffer']
        (if (and (seq bill)
                 (every? (fn [[k req]]
                           (>= (get buf k 0) req))
                         bill))
          (let [buf-next (reduce-kv
                          (fn [b k req]
                            (update b k - req))
                          buf
                          bill)]
